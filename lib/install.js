 ...
 */
async function cloneUpdatePackage( name, repo ) {
...
async function install(repos, options) {
	...
	for ( const repo of repos ) {
		const name = "TestName"; // TODO: work out from 'bower' URL
	...
	for ( const [name, repo] of repoMap ) {
		await cloneUpdatePackage( name, repo );
	}
	...
	for ( const name of newModules ) {
		if ( options.save ) {
			...
			caliberJson.dependencies[name] = repoMap[name];
		}
		if ( options['save-dev'] ) {
			caliberJsonChanged = true;
			...
			caliberJson.devDependencies[name] = repoMap[name];
		}
	}
